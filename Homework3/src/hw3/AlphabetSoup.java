...
public class AlphabetSoup
{
  ...
  public int[] getRowColumn(int rowOrColumn, Direction dir)
  {
    ...
    if (dir == LEFT) {
      for (int i = 0; i < size; i++) {
        temp[i] = grid[rowOrColumn][i];
      }
    }
    else if(dir == RIGHT) {
      for (int i = 0; i < size; i++) {
        temp[i] = grid[rowOrColumn][size - i - 1];
      }
    }
    else if(dir == UP) {
      for (int i = 0; i < size; i++) {
        temp[i] = grid[i][rowOrColumn];
      }
    }
    else if(dir == DOWN) {
      for (int i = 0; i < size; i++) {
        temp[i] = grid[size - i - 1][rowOrColumn];
      }
    }
    return temp;
  }
  ...
  public void setRowColumn(int[] arr, int rowOrColumn, Direction dir)
  {
    if (dir == LEFT) {
      ...
      for (int i = size; i > 0; i--) {
        grid[rowOrColumn][i - 1] = arr[size - i];
      }
      ...
      for (int i = size; i > 0; i--) {
        grid[i - 1][rowOrColumn] = arr[size - i];
      }
    }
  }
  ...
  public TileInfo setNewTile()
  {
    ...
    if(dir == DOWN) {
      ...
      int picker = rand.nextInt(0,empty.length);
      grid[0][picker] = config.randomNewTileValue(rand);
    }
